...
//  app.Id.regex = /=(.+)/; // match '=' and capture everything that follows
// app.matches = app.Id.match(app.Id.regex);
// console.log(app.mathches);
// if (app.matches) {
//      app.getId = app.matches[1];  // captured group, in this case, 
...
app.getId = app.Id.substr(app.Id.indexOf("=") + 1);
    ...
    this.regProfile = function(regData) {
        ...
        if (regData) {
            ...
            User.createProfile(app.regData).then(function(data) {
    ...
    app.regData.id = app.getId;
        // If form is valid and passwords match, attempt to create user         
        if (app.regData) {
            
            // Runs custom function that registers the user in the database 
            User.createBankDetails(app.regData).then(function(data) {
                // Check if user was saved to database successfully
                if (data.data.success) {
    ...
    this.regCar = function(regData) {
            ...
            User.createCarDetails(app.regData).then(function(data) {
                ...
                if (data.data.success) {
                    ...
                    $timeout(function() {
                          ...
                          $location.path( "/setbank/" +"user_id=" + data.data.id );
     ...
     this.regDocuments = function(regData) {
        app.disabled = true; // Disable the form when user submits to prevent multiple requests to server
        app.loading = true; // Activate bootstrap loading icon
        app.errorMsg = false; // Clear errorMsg each time user submits
        ...
        if (app.regData) {
            ...
            // Runs custom function that registers the user in the database 
            User.createDocuments(app.regData).then(function(data) {
                // Check if user was saved to database successfully
                if (data.data.success) {
                    ...
                    $timeout(function() {
                          ...
                          $location.path( "/getActivation" + data.data.id );
    ...
    this.doLogin = function(loginData) {
        ...
        Auth.login(app.loginData).then(function(data) {
            ...
            if (data.data.success) {
                ...
                $timeout(function() {
                    $location.path('/main'); // Redirect to home
